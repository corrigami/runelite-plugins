...
public class Motherlode {
    ...
    private final int UPSTAIRS_HEIGHT = -490;
    ...
    public Sector getVeinSector(final int x, final int y) {
    ...
    public int getPayDirtNeeded(final MotherlodeSack sack, final MotherlodeInventory inventory) {
        ...
        if (sack_pay_dirt == sack_size) {
            return inventory_size - inventory_items;

            // Can't put any more.
        } else if (sack_pay_dirt > sack_size) {
            return inventory_pay_dirt * -1;
        }

        // Can be changed.
        int to_mine = sack_size - sack_pay_dirt;
        if (to_mine > inventory_size) {
            return inventory_size - inventory_items;
        } else {
            return Math.min(to_mine - inventory_pay_dirt, inventory_size - inventory_items);
        }
    }
}
